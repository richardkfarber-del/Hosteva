#!/usr/bin/env python3
import json, os, sys
from gb_config import run_single_agent, local_config

print("======================================================")
print("  [PHASE 3] BACKLOG GROOMING")
print("======================================================")
print("-> AGENT-07-PRODUCT (Hawkeye) & Coulson bound to backlog_grooming_skill.md")

state_path = os.environ.get("SWARM_STATE_FILE", "../swarm_state.json")
try:
    with open(state_path, "r") as f:
        state = json.load(f)
except FileNotFoundError:
    state = {}

print("-> Executing GraphBit Node...")
result = run_single_agent("Backlog", "Hawkeye", "backlog_grooming_skill.md", local_config, state)
print("\n>>> [PHASE 3 OUTPUT]:\n", result)
print(">>> [ORCHESTRATOR]: Phase 3 Complete.")
sys.exit(0)
