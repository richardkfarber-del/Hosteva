...
| `tests/test_bug_pl08_street_view_object_storage.py` | BUG-PL-08 B+C: SV/Places → S3-compatible object storage (R2 default); no ephemeral durable writes; list lazy heal/migrate; admin backfill; fail-closed placeholder; mocked storage+fetch | api/unit | PR / main CI | CI on main is the gate | PL-08 Street View disk |
