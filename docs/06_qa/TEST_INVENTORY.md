...
| `tests/test_bug_pl11_compliant_badge.py` | BUG-PL-11: list hydrate + create align + badge honesty; mock UNDER_REVIEW must not return Compliant green path; ALLOWED_WITH_CHECKLIST→Action Required; dashboard resolveDashboardBadge markers | api/dom-assert | PR / main CI | CI on main is the gate | PL-11 Compliant badge |
...
| `tests/test_tampa_bay_seed_pasco_upsert.py` | Tampa Bay seed vs existing Pasco `JURISDICTION-RULES`: no UniqueViolation; tax_registration_url lands; no CUP force on tax-only live row; pack continues | unit | PR / main CI | CI on main is the gate | Pasco Curated pack upsert |
...
| `tests/test_us018c_address_normalize.py` | US-018c/TE-011 normalize-before-match: short≡full Stable Run; prevent 409 both forms; cleanup residual pair; distinct streets + cross-host intact | api/unit | PR / main CI | CI on main is the gate | PL-DUP US-018c |
...
**Module count:** 39 (`2` under `app/tests/`, `37` under `tests/`).
