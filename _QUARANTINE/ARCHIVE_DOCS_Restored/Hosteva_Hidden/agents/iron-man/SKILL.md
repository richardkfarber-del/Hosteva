...
1. **STEP 1:** Use the `edit` or `exec` tool to physically modify the target codebase file (e.g., `Dockerfile`, python scripts).
...
4. **STEP 4 (MANDATORY STAGING):** You MUST use the `exec` tool to run `git add <files>` to stage your changes. DO NOT run `git commit`. You must leave the physical files in the Git staging area for Heimdall.
